...
off_t find_offset(const char* path) {
    ...
    while (token != NULL) {
        ...
        for (int i = 0; i < superblock->num_inodes; i++) {
            ...
            if (bit != 0) {
                ...
                // printf("%d\n",current_inode->num);
                // printf("inode %d\n", i);
                if (current_inode->num == inode_offset) {
                    ...
                    for (int j = 0; j < D_BLOCK; j++) {
                        ...
                        if (offset != 0) {
                            ...
                            if(current_entry != NULL) {
                                ...
                                // printf("comparing this %s and this %s\n", current_entry->name, token);
                                if (!strcmp(current_entry->name, token)) {
                                    return (offset + k * sizeof(struct wfs_dentry));
                                    // printf("next inode num: %d\n", current_entry->num);
                                    found = 1;
                                    break;
...
static int wfs_getattr(const char* path, struct stat* stbuf) {
    ...
    if (temp_node == NULL) {
        printf("LEAVING:     getattr. file %s doesn't exist.and num returned %d\n", path,num);
        return -ENOENT;
    }
...
struct wfs_inode * allocate_parent(struct wfs_inode * child) {
    struct wfs_inode* possible_parent;
    uint8_t bit;
    off_t offset;
    struct wfs_dentry* current_entry;
    ...
    for (int i = 0 ; i < superblock->num_inodes ; i++) {
        bit = disk_image[superblock->i_bitmap_ptr + (i / 8)] & (1 << (i % 8));
        // Inode exists
        if (bit != 0) {
            
            possible_parent = (struct wfs_inode*) (disk_image + (superblock->i_blocks_ptr + i * BLOCK_SIZE));
            for (int j = 0 ; j < N_BLOCKS ; j++) {
                offset = possible_parent->blocks[j];
                current_entry = (struct wfs_dentry*) (disk_image + offset);
                if ((current_entry != NULL) && (current_entry->num == child->num)) {
                    disk_image[superblock->d_bitmap_ptr + (possible_parent->num)] = (disk_image[superblock->d_bitmap_ptr + (current_entry->num)] | (1 << (j)));
...
// off_t find_offset(struct wfs_inode * child) {
//     struct wfs_inode* possible_parent;
//     uint8_t bit;
//     off_t offset;
//     struct wfs_dentry* current_entry;
...
//                 offset = possible_parent->blocks[j];
//                 current_entry = (struct wfs_dentry*) (disk_image + offset);
//                 if ((current_entry != NULL) && (current_entry) (current_entry->num == child->num)) {
//                     // disk_image[superblock->d_bitmap_ptr + (current_entry->num)] = (disk_image[superblock->d_bitmap_ptr + (current_entry->num)] | (1 << (j)));
//                     return offset;
//                 }
//             }
//         }
...
struct wfs_dentry * write_entry(const char *path, struct wfs_inode * node) {
    ...
    if (strlen(directory) < 1) {
        ...
        if (index == 0) allocate_parent(parent_node);
...
static int wfs_read(const char* path, char *buf, size_t size, off_t offset, struct fuse_file_info* fi)
 {
    ...
    // struct wfs_inode * parent = allocate_parent(node); 

    off_t block = find_offset(basename((char*)path));
    if (block != 0) {
        char* file_content = (char*) (disk_image + block + sizeof(struct wfs_dentry) + offset);
            size = sizeof(file_content);
            memcpy((char*)buf,file_content,size);
            printf("this is what I read %s\n", buf);
            return size;
    }

    // for (int j = 0; j < D_BLOCK ; j++) {
    //     block = parent->blocks[j];
    //     if (block != 0) {
...
static int wfs_write(const char *path, const char *buf, size_t size, off_t offset, struct fuse_file_info *fi) {
    ...
    if (node->size == 0) {
        node->size = size;
        allocate_parent(node);
        
    }

    off_t block = find_offset(path) + sizeof(struct wfs_dentry);
    // off_t block = (off_t) 0;

    printf("block offset %ld dblocks offset %ld\n", block, (off_t) superblock->d_blocks_ptr);

    char* file_content = (char*) (disk_image + block + offset);
    memcpy(file_content,(char*)buf,size);
